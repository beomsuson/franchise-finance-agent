...
def ask_customer(question: str) -> str:
    ...
    감수 가능) 척도로 답하는 질문이 기본이다. 후보 간 선호도를 묻는 질문은 화면이 "후보 1"/"후보 2"
    같은 선택 버튼으로 바꿔서 보여주므로(자유 서술형 답변이 아님), 이 시스템 프롬프트에 적힌 대로
    반드시 "이 중 어느 후보를 더 선호하시나요?"로 질문을 끝맺어야 한다. 질문 문장에 실제 브랜드
    상호명을 절대 넣지 마라 — 고객이 브랜드명만 보고 선입견으로 답하지 않도록 "후보 1은 ~한
    조건인데" 식으로 번호로만 지칭해서 조건을 설명해야 한다."""
...
2) 후보 1개당 정확히 2개의 질문을 만드세요 — 같은 항목(예: 위약금)만 반복하지 말고, 계약위험
   ...
   근거로 쓰세요.
3) 후보가 2개 이상이면, 각 후보의 2개 질문이 끝난 뒤 선호도 비교 질문도 최소 1번 하세요. 이때
   "어느 쪽을 더 선호하시나요"라고만 물으면 고객이 무엇을 기준으로 답해야 할지 알 수 없으니,
   반드시 질문 문장 안에 각 후보에게 이미 물었던 구체적인 조항·조건 내용을 요약해서 나란히
   보여주세요 — 예: "후보 1은 위약금이 잔여 계약기간 월매출의 20%, 필수구매 품목이 있음 /
   후보 2는 위약금 조항 없음, 대신 폐업·해지율이 다소 높음" 처럼, 숫자 지표(창업비용 등)가
   아니라 방금 물어봤던 실제 계약위험/운영부담/지원제도 조항 내용을 근거로 비교해서 보여준 뒤
   반드시 "이 중 어느 후보를 더 선호하시나요?"라는 문장으로 끝맺으세요(화면이 이 표현을 보고
   답변 UI를 후보 선택 버튼으로 바꾸므로, 다른 표현으로 바꾸지 말고 이 문구를 그대로 쓰세요).
   고객이 직접 비교할 근거 없이 추상적으로 "선호도"만 묻지 마세요.
...
def select_brand_node(state: AgentState) -> dict:
    ...
    chosen = next((c for c in state["final_candidates"] if c["disclosure_id"] == disclosure_id), None)
    if chosen is None:
        # agent_driven_node가 재실행(replay)될 때 추론 모델의 도구 호출 순서가 이전과 완전히
        # 똑같지 않으면, 이 interrupt에 도달하는 시점이 어긋나면서 엉뚱한 값(예: 이전 질문의
        # 자유 텍스트 답변)이 disclosure_id로 잘못 들어올 수 있다. 그래프 전체를 죽이는 대신
        # 첫 번째 후보로 안전하게 폴백하고, 화면에 무슨 일이 있었는지 알 수 있게 표시한다.
        chosen = state["final_candidates"][0]
        disclosure_id = chosen["disclosure_id"]
